...
function onClickBotton () { document.querySelector('#operaton-buttons').addEventListener('click', operation => {
  const id = operation.target.getAttribute('data-id');
}) 
if (plus.addEventListener('click')) {
let result = Number(inpuT1) + Number(inpuT2);
alert(result);
} else if (minus.addEventListener('click')) {
  let result = Number(inpuT1) - Number(inpuT2);
alert(result);
} else if (minus.addEventListener('click')) {
  let result = Number(inpuT1) * Number(inpuT2);
...
buttons.forEach((element) => {
  element.addEventListener('click', onClickBotton);
})